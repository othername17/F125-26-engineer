...
def analyze_braking_and_entry(df):
    ...
    if not front_lockup.empty:
        ...
        st.info("Suggestion: Move brake bias rearward (e.g., 58% to 56%).")
    ...
    if not rear_lockup.empty:
        ...
        st.info("Suggestion: Move brake bias forward.")
    ...
    if not entry_understeer.empty:
        ...
        st.info("Suggestion: Increase Front Wing Aero.")
...
def analyze_corner_exit(df):
    ...
    if not power_oversteer.empty:
        ...
        st.info("Suggestion: Soften Rear Suspension; decrease On-Throttle Differential.")
    ...
    if not on_throttle_understeer.empty:
        ...
        st.info("Suggestion: Increase On-Throttle Differential.")
    if not issues_found: st.success("No Corner Exit & Traction issues detected.")
...
def analyze_aerodynamics(df):
    ...
    if not high_drag.empty:
        ...
        st.info("Suggestion: Reduce Front and Rear Wing angles.")
...
def analyze_chassis_dynamics(df):
    ...
    if not bottoming_out.empty:
        st.warning(f"⚠️ Bottoming Out Detected")
        st.info("Suggestion: Increase Ride Height.")
    ...
    if not kerb_instability.empty:
        st.warning(f"⚠️ Kerb Instability Detected")
        st.info("Suggestion: Soften Suspension and ARBs.")

def analyze_tyres(df):
    st.subheader("6. Tyre Temperatures & Wear")
    if df[['tyre_temp_0', 'tyre_temp_1', 'tyre_temp_2', 'tyre_temp_3']].max(axis=1).max() > 105:
        st.warning("⚠️ Tyre Overheating Detected")
        st.info("Suggestion: Lower Tyre Pressures.")
    if df[['tyre_wear_0', 'tyre_wear_1', 'tyre_wear_2', 'tyre_wear_3']].max(axis=1).max() > 50:
        st.warning("⚠️ High Tyre Wear Detected")
        st.info("Suggestion: Reduce Camber and Toe.")

def analyze_brake_temperatures(df):
    st.subheader("7. Brake Temperatures")
    if df[['brake_temp_0', 'brake_temp_1', 'brake_temp_2', 'brake_temp_3']].max(axis=1).max() > 1000:
        st.warning("⚠️ Brake Overheating Detected")
        st.info("Suggestion: Open Brake Ducts.")
    elif df[['brake_temp_0', 'brake_temp_1', 'brake_temp_2', 'brake_temp_3']].max(axis=1).max() < 400:
        st.warning("⚠️ Cold Brakes Detected")
        st.info("Suggestion: Close Brake Ducts.")
...
def analyze_fuel_and_weight(df):
    ...
    if (df['speed_kmh'].max() * 0.9 > df['speed_kmh'].min()) and (df['fuel'].max() > 10.0):
        ...
        st.info("Suggestion: Underfuel for stint.")
...
if uploaded_file is not None:
    ...
    if telemetry_df is not None:
        ...
        try: clean_track_id = int(float(raw_track_id))
        except: clean_track_id = raw_track_id
        ...
        if analyze_data_integrity(telemetry_df):
            ...
            if 'velocity_delta' in telemetry_df.columns:
                st.subheader("11. Velocity Delta")
                delta_fig = go.Figure()
                delta_fig.add_trace(go.Scatter(x=telemetry_df.index, y=telemetry_df['velocity_delta']))
